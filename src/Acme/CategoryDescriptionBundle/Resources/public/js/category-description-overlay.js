// public/bundles/acmecategorydescription/js/category-description-overlay.js
(function () {
  ...
  function waitForPropertiesContainer(timeoutMs = 8000) {
    return new Promise((resolve, reject) => {
      ...
      const check = () => {
        ...
        if (performance.now() - t0 >= timeoutMs) return reject(new Error('container timeout'));
        requestAnimationFrame(check);
      };

      // In case it appears due to async render, also observe the document
  ...
  async function mountForCurrentRoute() {
    ...
    // If we changed category, rebuild card to avoid stale listeners
    ...
    // Wait until properties container *really* exists, then insert there
    try {
      ...
      // If it didn’t appear in time, try again shortly
      setTimeout(mountForCurrentRoute, 400);
      return;
    }

    // Hydrate once per route change (don’t clobber user typing)
    if (currentId !== id) {
      currentId = id;
      try {
        ...
        if (imgUrl) { if (preview.src !== imgUrl) preview.src = imgUrl; preview.style.visibility = 'visible'; }
  ...
  // Small heartbeat: re-attach if React recreates the container later
  setInterval(() => {
    if (isTyping) return;
    ...
    // Ensure card is still inside the container
    const id = getRouteCategoryId();
    if (!id) return;
    const card = document.querySelector('#acme-description-card');
    ...
    // If image preview is missing on first paint, hydrate it
    if (card) {
      const preview = card.querySelector('[data-acme="image-preview"]');
